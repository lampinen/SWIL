...
class MNIST_autoenc(object):
    ...
    def base_train(self, dataset, nepochs=100):
    ...
    def new_data_train(self, new_dataset, old_dataset=None, nepochs=100):
        ...
        for epoch in range(nepochs):
            ...
            for batch_i in xrange(len(new_dataset["labels"])//batch_size):
                if self.replay_type == "None":
                    ...
                    probabilities = softmax(np.amax(dots, axis=1)) 
                    print(probabilities, np.shape(probabilities))
                
                    # note that this is actually pretending that sampling with
                    # replacement is sampling without, but on a dataset this 
                    # large that shouldn't really be an issue
