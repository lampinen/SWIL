...
```{r}
ggsave("results/plots/summarized_train_losses.png", width=10, height=10) 
```

```{r}
ggplot(summarized_loss_d %>% filter(epoch < 25 & epoch >= 20), aes(x=epoch, y=mean_loss, color=left_out_class)) +
  geom_line() +
  ylim(40, 60) +
  facet_wrap( ~replay_type +temperature + SW_by)
```
```{r}
ggsave("results/plots/summarized_losses_20_24.png", width=10, height=10) 
```
