...
temperatures = c(0.0, 0.1, 1, 10, 100, 100)
    ...
    for (replay_type in replay_types) {
      for (temperature in temperatures) {
        ...
        for (SW_by in SW_bys) {
          ...
          if (replay_type == "SWIL") {
            filename_prefix = sprintf("run%i_lo%i_m_%s_swby_%s_t_%.1f_", run, lo, replay_type, SW_by, temperature)
          } else {
...
```{r}
ggsave("results/plots/encountered_lo0.png", width=10, height=10) 
```
...
```{r}
ggsave("results/plots/encountered_lo5.png", width=10, height=10) 
```